...
gato.prototype.toString = function () {
  return "El gato se llama: " + this.nombre + " y tiene " + this.edad;
};
...
//Interface iterator
//un objeto x es iteralbe si tiene la propiedad
obj[Symbol.iterator];
//la cual es una funcion que devuelve un iterador
//el iterador tiene un metodo next
next();
//este debe devolver value y done
...
const obj = {
  ...
  [Symbol.iterator]() {
    ...
    return {
      next: () => {
        // al ser una funcion arrow hereda el this de donde fue creada
...
// con este se podria decir que hemos creado nuestro propio for of
// lo que implica que podemos hacer que cualquier estructura de datos creada pueda hacer
// uso del for of
// otro ejemplo con una estructura propia
class Nodo {
  constructor(valor, nodoSiguiente) {
    this.value = valor;
    this.nodoSiguiente = nodoSiguiente;
  }
  get length() {
    return 1 + (this.nodoSiguiente ? this.nodoSiguiente.length : 0); //recursividad en el metodo get
  }
...
class NodoIterator{
  ...
  next(){
    ...
    let valor=this.nodo.value;
    this.nodo = this.nodo.nodoSiguiente;
    return{value: valor,done:false}
  }
}
Nodo.prototype[Symbol.iterator]= function(){
  return new NodoIterator(this);//crear el iterator tomandose a si mismo como para parametro del constructor
}
let lista = Nodo.fromNumbers(1,2,3,4,5,6);
for(let e of lista){
  console.log(e);
}//=>
//1 
